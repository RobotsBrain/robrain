...
void Mp4Encoder::Start(const char *filename, int width, int height, int vtimescale, int framerate, int atimescale)
{
	if(filename == NULL)
		return;
...
int Mp4Encoder::WriteH264Data(u_char *pData, int size)
{
	...
	if (type == 0x07) {	// sps  
		...
		MP4AddH264SequenceParameterSet(m_hMp4File, m_videoId, pData + 4, size - 4);
	} else if (type == 0x08) {	// pps		
		MP4AddH264PictureParameterSet(m_hMp4File, m_videoId, pData + 4, size - 4);
	} else {
