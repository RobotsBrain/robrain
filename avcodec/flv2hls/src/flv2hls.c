...
static int hls_append_aud(str_buf_t * out)
{
...
static int hls_append_sps_pps(av_codec_ctx_t*codec_ctx, str_buf_t*out)
{
    u_int8_t                        *p;
    u_int8_t                        *in;
    hls_ctx_t             *ctx;
...
static int hls_audio(Flv2hlsContext*context, u_int8_t*data, int data_len, u_int32_t timestamp)
{
    hls_ctx_t                   *ctx;
    av_codec_ctx_t           *codec;
