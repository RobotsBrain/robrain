...
static int flv_mpegts_write_file(flv_mpegts_file_t *file, u_char *in,
    size_t in_size)
{
    ssize_t   rc;
...
static int32_t flv_mpegts_write_header(flv_mpegts_file_t *file)
{
...
static u_char *flv_mpegts_write_pcr(u_char *p, u_int64_t pcr)
{
    *p++ = (u_char) (pcr >> 25);
    *p++ = (u_char) (pcr >> 17);
    *p++ = (u_char) (pcr >> 9);
...
static u_char *flv_mpegts_write_pts(u_char *p, u_int32_t fb, u_int64_t pts)
{
    u_int32_t val;
...
int flv_mpegts_write_frame(flv_mpegts_file_t *file,
...
int32_t flv_mpegts_open_file(flv_mpegts_file_t *file, char *path)
{
